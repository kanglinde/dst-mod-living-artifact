...
local function onspawn(inst)
	local ent = inst.target
	if ent and ent:IsValid() then
		local x, y, z = inst.Transform:GetWorldPosition()
		local ex, ey, ez = ent.Transform:GetWorldPosition()
		inst:ForceFacePoint(ex, ey, ez)
...
local function sparklefn()
	...
	-- to make locomotor work
	MakeTinyFlyingCharacterPhysics(inst, 1, 0.3)
...
return Prefab("lifeplant", fn, assets),
       Prefab("lifeplant_sparkle", sparklefn, assets)